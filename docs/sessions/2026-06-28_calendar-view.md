...
- Playwright で全ページの横オーバーフローを検査（pageOverflow=0 で横スクロールは無し）。長いメモのとき収支行の「金額・編集/削除」が右に押し出されてカードの overflow-hidden で見切れる不具合を発見・修正。原因はカラーエッジ横の `flex flex-1` ラッパに `min-w-0` が無くメモの truncate が効かなかったこと。収支一覧・カレンダー明細・定期項目の3箇所に `min-w-0` を付与。

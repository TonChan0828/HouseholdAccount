...
ログイン後の起点画面（`/dashboard`）。アクティブな household の「当期間」の収支を一目で把握する。
月次サマリー（収入計・支出計・収支差・予算進捗）、当期収支グラフ、メンバー別カテゴリマトリクス、最近の取引を表示し、
...
- サマリーカード: 収入計（緑）・支出計（赤）・収支差。予算合計が設定済み（>0）のときは予算進捗バー（世帯全体の当期支出 / 予算合計）を併記する（`budgetTotal`/`budgetSpent` props。scope に依らず全体実績で対比。30_budget_management.md）

...
- 進捗（貯金額）= 開始日 `start_date` 以降の**世帯全体**の `(収入合計 − 支出合計)`。
  - scope（mine/all）には依存せず、常に世帯全体で計算する。
  - 貯金額が負（支出超過）になりうる。その場合は進捗0%・残額は目標全額として扱う。
  - 集計に「今日まで」の上限は付けない。サーバの UTC 基準「今日」とユーザーのローカル日付がずれると当日分が除外されるため（開始日以降を全て対象とする）。
...
- **目標設定済み**:
  ...
  - 「編集」「解除」操作。
- **達成時の演出**（`reached`）:
  - 暖色（`accent-warm` ＝ゴールド）のグロー＋リング、トロフィーアイコン付き「🎉 目標達成！」バッジ（`yen(saved)` 貯まりました）。
  - 進捗バーはゴールドのシマー（光沢スイープ）に切り替わる。
  - 紙吹雪を 1 セッションにつき一度だけ再生（`components/features/dashboard/goal-celebration.tsx`）。`prefers-reduced-motion` 有効時・matchMedia 非対応環境では再生しない。

...
- E2E は共有ユーザー（`e2e@e2etest.dev`）の表示名を変更するため、テスト末尾で必ず "e2e" に復元する。`members.spec.ts` は表示名の値に依存しない形に修正済みだが、表示名を文言で検証する E2E を今後追加する場合は競合に注意する
- E2E 実行のたびに家計簿グループが蓄積し、/households が肥大化して並列実行が不安定になる（2026-06-13 に残骸169件を削除して解消）。定期的なクリーンアップか teardown の仕組みが将来課題

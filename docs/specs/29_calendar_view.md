...
- **デスクトップ構成（lg 以上）**: `MainSideGrid`（`components/shared/main-side-grid.tsx`）で
  メイン=カレンダーグリッド（7/12）/ サイド=当月サマリー+選択日明細（5/12）の2カラム。
  日をタップすると右レールの明細が即時更新され、スクロール不要で確認できる。
  コンテナは `lg:max-w-6xl`。月ナビ・リスト⇄カレンダー切替は PageHeader の actions に置く。
  lg 未満は従来どおり サマリー → グリッド → 明細 の縦積み（`max-lg:order-*` で制御）。

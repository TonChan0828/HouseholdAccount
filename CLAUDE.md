...
app/
  ...
  (dashboard)/          # 認証済みルート（household_id コンテキスト必須）
    ...
    categories/         # カテゴリ管理
    analytics/          # 月次グラフ・カテゴリ別内訳・家計アドバイス
...
docs/specs/
  ...
  31_analytics_advice.md # 家計アドバイス（ルールベース）（分析画面上部に予算/トレンド/集中度/貯蓄率から自動生成した助言を表示）
  32_savings_goal.md     # 貯金目標（グループに1つの目標額・開始日以降の収支差額で進捗を自動算出・期日は任意でペース表示・ダッシュボード表示）
  33_month_end_forecast.md # 月末着地予測（ダッシュボード・当期のみ・着地収支と予算超過警告）
  34_dashboard_two_column_layout.md # ダッシュボード2カラム構成（デスクトップでメイン7:サイド5・モバイルは現行維持）
  35_architecture_refactor.md # アーキテクチャリファクタリング（認証ガード・メンバー名解決・期間クエリの集約と定期収支生成の呼び出し漏れ修正）
```
...
### Rule 5 — E2E 全通過（機能実装の完了条件）

機能実装が「完了」と言えるのは、E2E テストが**全て通過**していることを確認できた時のみ。実装が一段落しても、E2E を実行して全テストがグリーンになるまで完了とはみなさない。

手順:

1. 機能実装と Unit/Component テスト（Rule 1）が揃ったら `npm run test:e2e` を実行する
2. 全テストが通過することを実際の出力で確認する（「通るはず」で済ませない）
3. 失敗があれば原因を修正し、再実行する。全通過するまで繰り返す
4. 全通過を確認できて初めて、機能完了・コミット可能とする

新機能を追加した場合は、その機能を検証する E2E テストも併せて追加する。

#### ユーザーストーリー駆動の E2E（毎回実施）

E2E は「仕様 → ユーザーストーリー → それを検証する E2E」の対応で管理する。対応表は
[`docs/user-stories.md`](docs/user-stories.md)。**仕様を追加・変更したら毎回**、次を行う:

1. 対象仕様からユーザーストーリー（As a … / I want …）を起こし、`docs/user-stories.md` に追記/更新する
2. そのストーリーが正常に動作することを確認する E2E を作成/更新する（実ブラウザでのユーザー操作で検証）
3. `npm run test:e2e` を**CI同条件（`--workers=1`）で全通過**するまで実行する
4. E2E が困難・過剰な領域（実メール送受信・OCR・破壊的な退会など）は、Unit/Component または手動確認で代替し、対応表に検証方法と根拠を明記する（無理に脆い E2E を作らない）

注: ローカルの多ワーカー並列はグループ作成系テストが `active_household_id` を共有して競合する。全通過の判定は `--workers=1`（CI と同条件）で行う。
...
5. **月次分析**: 月別推移グラフ + カテゴリ別円グラフ + 家計アドバイス（メンバー別切り替えは未実装・機能07連携の将来課題）
